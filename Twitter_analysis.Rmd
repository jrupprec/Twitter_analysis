...
### inspect x-variables
```{r}
ggplot(complete_df, aes(x = followers_count))+
  geom_density()+ # extreme kurtosis 
  labs(title = "Density: Number of Followers")

ggplot(complete_df, aes(x = friends_count))+
  geom_density()+ # extreme kurtosis 
  labs(title = "Density: Number of Friends")

complete_df_hashtags <- complete_df %>%
  count(no_hashtags) %>%
  mutate(perc = n/nrow(complete_df))

ggplot(complete_df_hashtags, aes(x = no_hashtags, y = perc)) +
  geom_bar(stat = "identity") +
  xlim(-1,10) +
  labs(title = "Share of Number of Hashtags")

complete_df_mentions <- complete_df %>%
  count(no_mentions) %>%
  mutate(perc = n/nrow(complete_df))

ggplot(complete_df_mentions, aes(x = no_mentions, y = perc)) +
  geom_bar(stat = "identity") +
  xlim(-1,10) +
  labs(title = "Share of Number of Mentions")
```
...
complete_df <- complete_df %>% 
  mutate(log_retweets = log(retweet_count)) %>%
    filter(!if_all(c( "mean_sentiment_party", "mean_sentiment_tweet", "number_positive_words", "number_negative_words", "log_retweets"), is.na)) %>%
  filter(!if_all("log_retweets", is.infinite))

# observe data of retweets (resembles log-normal distribution)
ggplot(complete_df, aes(x = log_retweets))+
  geom_density() +
  xlim(0,10) # extreme kurtosis

ggplot(complete_df, aes(x = log_retweets))+
  geom_histogram(bins = 50) + # extreme kurtosis 
  xlim(-1,5)
...
set.seed(20)
sample <- sample(c(TRUE, FALSE), nrow(complete_df), replace = TRUE, prob = c(0.8,0.2))
data_tweets_test <- complete_df[!sample,]
complete_df <- complete_df[sample,]

rt_model_poi <- glm(retweet_count ~ followers_count + statuses_count + friends_count + mean_sentiment_party + mean_sentiment_tweet + no_hashtags + no_mentions, family = "poisson", data = complete_df)


summary(rt_model_poi)

library(car)
vif(rt_model_poi) # no multicollinearity problem
```

```{r fitted values}
data_tweets_test$pred.retweet_count_poi <- round(predict(rt_model_poi, newdata = data_tweets_test, type = "response"))
```

```{r}
# standardized residuals vs. fitted plot
plot(rt_model_poi, which = 1, main = "Standardized Residuals Plot")


std.resid <- rstandard(rt_model_poi) 
...
```{r accuracy measurement}
library(msme)
P__disp(rt_model_poi) # very bad fit
# if the dp<1 your model is good;conversively, if the dp>1 the model is not fitting well your data

# -> sign of overdispersion, therefore use negative binomial
```

```{r negative binomial}
rt_model_nb <- glm.nb(retweet_count ~ followers_count + statuses_count + friends_count + mean_sentiment_party + mean_sentiment_tweet + no_hashtags + no_mentions, data = complete_df)

summary(rt_model_nb)
vif(rt_model_nb) # no multicollinearity problem


pchisq(2 * (logLik(rt_model_nb) - logLik(rt_model_poi)), df = 1, lower.tail = FALSE)
...
```{r fitted values nb}
data_tweets_test$pred.retweet_count_nb <- round(predict(rt_model_nb, newdata = data_tweets_test, type = "response"))

# standardized residuals vs. fitted plot
plot(rt_model_nb, which = 1, main = "Standardized Residuals Plot")
```
